...
- **White everywhere, no divider lines** — no tinted section backgrounds and no hairlines between sections, list items or grid cells; generous whitespace does the separating (`Section` = `py-24 sm:py-32 lg:py-40`, `Statement`/`CtaBand` larger). `line` is only for form-field borders; `mist` for the footer, image placeholders and brand-kit tiles.
...
- **Logo**: two linked rings (cobalt + navy, rising 10°) — exact arc paths in [components/brand/Logo.tsx](components/brand/Logo.tsx), also `public/brand/` (`hitroo-logo.svg/.png` — the lockup with the name as outlines, `hitroo-mark.svg/.png`, `hitroo-brand-kit.zip`; the `/brand` page shows the rules), `public/favicon.svg` (switches colours in dark mode) and the PNG/ICO favicon set. Always blue and black on white or a light background — the site never uses a reversed/light logo (the footer ends on white). Always shown with the HITROO name except as favicon/app icon; keep the rings open and never red/yellow/orange (trademark distance from Mastercard-style marks).
...
- **Imagery**: realistic, professional corporate-style photos generated with Codex (clean, bright, modern: the "Professional photo set" style line in [docs/art/photos-briefs.md](docs/art/photos-briefs.md)), in `public/photos/*.webp`. No people, no readable text, no brand-colour grading. The home story cards use a cinematic night set (`story-*.webp`, navy shadows, warm practical light). Never fake UI mocks, fake cursors, dotted grids or glows — they read as AI output.

### Pages (App Router)
Marketing pages live in the route group `app/(site)/` whose [layout](app/(site)/layout.tsx) renders the shared Header + `<main id="main">` + Footer once. Pages are **server components**; interactivity is isolated in client islands. Routes: `/`, `/services`, `/services/[slug]`, `/insights`, `/articles`, `/blog`, `/articles/[slug]`, `/blog/[slug]`, `/news`, `/news/[slug]`, `/ai-perspective`, `/about`, `/research`, `/support`, `/careers`, `/contact`, `/brand`, `/privacy`, `/cookies`, `/terms`, `/accessibility`, `/security`, `/site-map` (plus `public/.well-known/security.txt` and the RSS feed `app/feed.xml`). The group layout also mounts `Analytics` (first-party page views, clicks and reading) and `CookieConsent`. There is no admin here any more; [app/not-found.tsx](app/not-found.tsx) renders its own Header/Footer. Metadata comes from `pageMetadata()` in [lib/seo.ts](lib/seo.ts) (per-page canonical — never set one in the root layout) and JSON-LD from the builders there, rendered with `components/seo/JsonLd`. Discovery: [app/sitemap.ts](app/sitemap.ts), [app/robots.ts](app/robots.ts), `app/llms.txt` and `app/llms-full.txt` (generated from site data and posts).
...
- `PageHero` (inner pages sit close under the header; `spacious` keeps the home page's air), `CtaBand`, `ServiceGrid` (open grid of service icons + short names), `ContactBlock`, `BrandIcon` (the Codex icon set in `public/icons/`), `LegalSections` (body of the policy pages).
- Home: `StoryCarousel` (client; cinematic full-bleed photo cards, serif headline, arrows and dots), `ProcessAccordion` (client; "Fast, by design" — six photo panels, the open one wide, hover/focus/tap opens).
- `/brand`: `ColorSwatch` (client; copies the hex).
- `FeaturedQuote` (home: Sundar Pichai's words from Google Cloud Next 2026 on an ink card with a freely licensed black-and-white portrait, the source linked and the photo credit always shown; `HOME_QUOTE`), `LeaderQuote` (About: Rohit's words with his small round photo; `FOUNDER`), `FaqList` (questions in native `<details>`, paired with `faqLd()`).
- `Header` (server; builds the Services and Resources menus with icons) + `HeaderShell` (client; sticky white header, menus, mobile panel, active link), `Footer` (grey site map; ends with © and the legal row), `Wordmark`.
