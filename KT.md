...
- `/` Home: hero (photo) → "Why HITROO" two-line statement (+ link to our AI view) → "Why your business needs it" (Software / Automation / AI) → "What we build" → story cards (Our story / Our view on AI / Research) → "Fast, by design" (six photo panels) → Sundar Pichai's quote card ("We’ve seen how every employee in every organization can become a builder.", Google Cloud Next, April 2026) → "Support in one app" → "Who we work with" → enquiry form.
...
- **Quotes:** a public figure's exact words with the source linked, recent and positive (the owner found "Software is eating the world" negative and dated). A photo only if it is freely licensed for commercial use (CC BY, CC0 or public domain, e.g. from Wikimedia Commons) with the credit shown under it — never a press photo and never a generated likeness. Nothing on the page may suggest they endorse HITROO.
...
- `public/people/` — `rohit.webp` (the founder's own photo) and `sundar-pichai.webp` (Wikimedia Commons, Lukasz Kobus / European Commission, CC BY 4.0; cropped and made black and white — keep the credit line under the quote card).
