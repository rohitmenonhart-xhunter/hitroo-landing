    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="px-6 pt-36 md:pt-44 pb-20 overflow-hidden">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <Reveal>
            <span className="eyebrow text-[#0a0a0a]">Services</span>
                ...
                <Link href={`/services/${s.slug}`} className="group block border-b border-[#e5e5e5]" style={{ ['--c' as string]: s.color }}>
                  <div className="py-7 md:py-8 flex items-center gap-5 md:gap-8">
                    ...
                    <div className="flex-1 min-w-0 md:flex md:items-baseline md:justify-between md:gap-10">
                      <h2 className="text-2xl md:text-3xl font-semibold tracking-[-0.02em] text-[#0a0a0a] transition-transform duration-300 group-hover:translate-x-1.5">{s.title}</h2>
      ...
      {/* CTA */}
      <section className="px-6 py-32 md:py-40 text-center border-t border-[#e5e5e5]">
        <Reveal className="max-w-3xl mx-auto">
          <h2 className="text-4xl md:text-6xl font-bold tracking-[-0.03em] text-[#0a0a0a] leading-[1.0]">Have a project in mind?</h2>
          <p className="text-lg text-[#4a4a4a] mt-6">Tell us what you&apos;re building and we&apos;ll map the path to ship it.</p>
          <div className="flex items-center justify-center gap-4 mt-9">
            <Link href="/contact" className="inline-flex items-center gap-2 bg-[#0a0a0a] text-white text-sm font-medium px-8 py-4 rounded-full shadow-[0_8px_24px_rgba(32,33,36,0.25)] hover:bg-black hover:-translate-y-0.5 transition-all">Start a Project <ArrowRight className="h-4 w-4" /></Link>
            <Link href="/support" className="text-sm font-semibold text-[#4a4a4a] hover:text-[#0a0a0a] transition-colors">Get support</Link>
          </div>
        </Reveal>
      </section>
