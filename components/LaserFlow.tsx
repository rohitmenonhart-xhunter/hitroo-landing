...
#define DITHER_STRENGTH 1.0
    ...
    float bs(vec2 p,vec2 q,float powr){
        float d=distance(p,q)/uCoreWidth,f=powr*uFalloffStart,r=(f*f)/(d*d+EPS);
        return powr*min(1.0,r);
    }
    float bsa(vec2 p,vec2 q,float powr,vec2 s){
        vec2 d=(p-q)/uCoreWidth; float dd=(d.x*d.x)/(s.x*s.x)+(d.y*d.y)/(s.y*s.y),f=powr*uFalloffStart,r=(f*f)/(dd+EPS);
        return powr*min(1.0,r);
    }
    ...
    const float G=0.05; float xS=(1.0+(FLARE_AMOUNT*W_CURVE_AMOUNT*G)*cm)*uCoreWidth;
    float wHalf=W_HALF*uCoreWidth,wAA=W_AA*uCoreWidth;
    float sPix=clamp(y/R_V,0.0,1.0),bGain=pow(1.0-sPix,W_BOTTOM_EXP),sum=0.0;
    for(int s=0;s<2;++s){
        float sgn=s==0?-1.0:1.0;
        for(int i=0;i<W_LANES;++i){
            if(i>=lanes) break;
            float off=(W_BASE_X+float(i)*W_LAYER_GAP)*uCoreWidth,xc=sgn*(off*xS/uCoreWidth);
            float dx=abs(uv.x-xc),lat=1.0-smoothstep(wHalf,wHalf+wAA,dx),amp=exp(-off*W_SIDE_DECAY/uCoreWidth);
...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    uniforms.uCoreWidth.value = coreWidth;
  ...
  return <div ref={mountRef} className={`w-full h-full relative ${className || ''}`} style={{ width: '100%', height: '100%', minHeight: '100vh', ...style }} />;
};
